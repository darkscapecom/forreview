...
import { Plus, Trash2, ArrowLeft, Code, Copy } from 'react-feather'
import { CodePopup } from './CodePopup'
import { getUniqueDynamicID } from './utils'
import classNames from 'classnames'

export const AdminItems = ({highlight, setSelected, label, list = [], setList, def = false, preselected = false, keyAttribute, nameAttribute, newItem = {[keyAttribute]: 'item' + Date.now(), [nameAttribute]: 'New item'}, back}) => {
...
export const AdminItemSingle = forwardRef(({list, setList, selected, setSelected, keyAttribute, nameAttribute, render, def = false, samples = false, duplicate = false}, ref) => {
	...
	const deleteItem = () => {
		...
		setList(list.filter(i => i[keyAttribute] !== selected));
		setSelected(false);
	}

	const duplicateItem = () => {
		const newId = getUniqueDynamicID(list, selected);
		let item = {
			...getSelected(),
			id: newId
		}
		if (item.coord[0]) item = {...item, coord: [item.coord[0] + 0.01, item.coord[1]]};
		setList([item, ...list]);
	}
					...
					<div className="gap-2 panel-header" style={{padding: ' 16px 12px 0 12px'}}>
						...
						<div className="flex">
							{ duplicate && (
								<button
									type="button"
									className="mapplic-admin-button"
							...
							<button
								type="button"
								className="mapplic-admin-button warn"
								disabled={def}
								onClick={deleteItem}
							>
								<Trash2 size={16} />
